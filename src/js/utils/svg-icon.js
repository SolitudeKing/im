...
class SvgIconLoader {
    ...
    scanAndLoadIcons() {
        // 专门查找 i 标签
        const iElements = document.querySelectorAll(`${this.config.targetTag}[class*="${this.config.iconPrefix}"]`);

        iElements.forEach(element => {
            const classList = Array.from(element.classList);
            const iconClass = classList.find(cls => cls.startsWith(this.config.iconPrefix));
    ...
    async loadIconTo(iconName, target) {
        ...
        if (element.tagName.toLowerCase() !== this.config.targetTag) {
            console.warn(`Target element is not a ${this.config.targetTag} tag:`, element);
        }
    ...
    async loadIcons(icons) {
        const promises = icons.map(iconName => {
            const elements = document.querySelectorAll(`${this.config.targetTag}.${this.config.iconPrefix}${iconName}`);
            return Promise.all(
